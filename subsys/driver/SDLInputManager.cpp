...
EC_SDL_InputManager::EC_SDL_InputManager(irr::IrrlichtDevice* renderWindow) {
	...
	// Get window handle
	irr::video::SExposedVideoData d = renderWindow->getVideoDriver()->getExposedVideoData();
	switch (renderWindow->getVideoDriver()->getDriverType()) {
	case irr::video::EDT_OPENGL: windowHnd = reinterpret_cast<HWND>(d.OpenGLWin32.HWnd); break;
	case irr::video::EDT_DIRECT3D8: windowHnd = reinterpret_cast<HWND>(d.D3D8.HWnd); break;
	case irr::video::EDT_DIRECT3D9: windowHnd = reinterpret_cast<HWND>(d.D3D9.HWnd); break;
	}
